...
void list_teams(char **info)
{
    ...
    for (int i = 2; info[i] != NULL; i++) {
        ...
        team_name = get_msg_up_to_char(info[2], *SPLIT_LINE, (int)strlen(team_uuid) + 1);
        team_desc = get_msg_after_nb(info[2], (int)strlen(team_uuid) + (int)strlen(team_name) + 2);
...
void list_channels(char **info)
{
    ...
    for (int i = 2; info[i] != NULL; i++) {
        ...
        channel_name = get_msg_up_to_char(info[2], *SPLIT_LINE, (int)strlen(channel_uuid) + 1);
        channel_desc = get_msg_after_nb(info[2], (int)strlen(channel_uuid) + (int)strlen(channel_name) + 2);
...
void list_threads(char **info)
{
    ...
    for (int i = 2; info[i] != NULL; i++) {
        ...
        client_channel_print_threads(thread_uuid, user_uuid, (time_t)thread_timestamp, thread_title, thread_body);
...
void list_replies(char **info)
{
    ...
    for (int i = 2; info[i] != NULL; i++) {
        thread_uuid = get_msg_up_to_char(info[2], *SPLIT_LINE, 0);
        user_uuid = get_msg_up_to_char(info[2], *SPLIT_LINE, (int)strlen(thread_uuid) + 1);
        reply_timestamp = get_msg_up_to_char(info[2], *SPLIT_LINE, (int)strlen(thread_uuid) + (int)strlen(user_uuid) + 2);
        reply_body = get_msg_after_nb(info[2], (int)strlen(thread_uuid) + (int)strlen(user_uuid) + (int)strlen(reply_timestamp) + 3);
        client_thread_print_replies(thread_uuid, user_uuid, (time_t)reply_timestamp, reply_body);
        do_multiple_frees(thread_uuid, user_uuid, reply_timestamp, reply_body);
    }
}
